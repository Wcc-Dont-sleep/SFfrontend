...
const LogTable: FC<TableProps> = ({ logs, probability, threshold }) => {
  ...
  const handleSelectAllWarningInfos = (
    ...
    setSelectedLogs(event.target.checked ? logs.map((log) => log.time) : []);
  };
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>{t('异常分数')}</TableCell>
                    <TableCell>{t('日志内容')}</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {paginatedLogs.map((log) => {
                    const isLogSelected = selectedLogs.includes(log.time);
                    return (
                      <TableRow hover key={log.time} selected={isLogSelected}>
                        <TableCell>
                          <Box display="flex" alignItems="center">
                            <Checkbox
                              ...
                              onChange={(event) =>
                                handleSelectOneWarning(event, log.time)
                              }
                              value={isLogSelected}
                            />
                            <Box pl={1}>
                        ...
                        <TableCell>
                          <Typography 
                          color={threshold ? (
                            log.score >= threshold ? "error" : "success"
                          ) : "info"}
                          noWrap
                          >
                            {log.content}
